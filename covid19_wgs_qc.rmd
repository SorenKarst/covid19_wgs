...
# Batch and treatment are used interchangeably throughout the code. The treatment var was added to allow for an alternative grouping var for some plots.
...
|MQ|< 3000 N|
|n_filter|>= 3000 N|
|yield_filter|sample yield < `r yield_filter_factor`x negative control (max)|
...
seqsum_t2 <-
  ...
  left_join(
    m %>% select(-sample_category, -treatment),
...
* Barcode classification: Barcode has been registered as `used` for a sample in the metadata og not `unused`. Data in unused barcodes should not occur.
...
The search bar can be used to filter the data. For example search "<batch_id> Unused", to show all unused barcodes with data in a specific batch.

The columns can be sorted by presseing the small arrows next to the column names.
...
```{r message=FALSE, warning=FALSE, echo=FALSE, fig.height=3*treatment_n + 2}
acny <-
  acn %>%
  left_join(
    yf,
...
NB: Plot is only showing genomes with < 3000 Ns and the negative controls.
...
* >2 amplicon dropouts. Usually indicates sub-optimal PCR reaction conditions. Possible causes: error in reagent mixing/dispensing, lid problems or machine temperature control problems.
...
for (i in 1:length(tree)){
  ...
  p <- ggtree(bt, branch.length = "none") %<+% tm %>%
    {.} +
    geom_treescale() +
    geom_tiplab(
      align = T,
